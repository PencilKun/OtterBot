...
def text2img(text):
    ...
    msg = "[CQ:image,file=base64://{}]".format(base64_str)
    return msg
...
def GetRSSName(Uid,Site="bilibili"):
    try:
        if Site == "weibo":
            Url = f"https://rsshub.pencilss.top/weibo/user/{Uid}"
            R = feedparser.parse(Url)
            Name = R.get("feed").get("title","None").replace("的微博","").strip()
        else:
            Url = f"https://rsshub.pencilss.top/bilibili/user/dynamic/{Uid}"
            R = feedparser.parse(Url)
            Name = R.get("feed").get("title","None").replace("的 bilibili 动态","").strip()
    except Exception as e:
        logging.error(f"Func_GetRSSName_Error:{e}")
        Name = "None"
    return Name
